...
# boot_decks, boot_deck, midi_relay, warm_jit) and every qemu, without pkill -f matching this shell.
for pat in run/live.sh run/live_linked.sh run/warm_jit.sh run/warm_jit.py run/rig.sh run/play_real_dsp.sh run/boot_decks.sh run/boot_deck.sh run/midi_relay.py; do
